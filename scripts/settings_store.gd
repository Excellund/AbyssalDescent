...
const TIMER_VISIBLE_KEY := "timer_visible"
...
const DEFAULT_TIMER_VISIBLE := true
...
static func load_settings() -> Dictionary:
	...
	if err != OK:
		return {
			...
			"timer_visible": DEFAULT_TIMER_VISIBLE,
	...
	return {
		...
		"timer_visible": bool(config.get_value(UI_SECTION, TIMER_VISIBLE_KEY, DEFAULT_TIMER_VISIBLE)),
...
static func save_settings(master_volume_db: float, music_volume_db: float, sfx_volume_db: float, resolution_width: int = DEFAULT_RESOLUTION_WIDTH, resolution_height: int = DEFAULT_RESOLUTION_HEIGHT, display_mode: String = DEFAULT_DISPLAY_MODE, telemetry_upload_enabled: bool = DEFAULT_TELEMETRY_UPLOAD_ENABLED, telemetry_consent_asked: bool = DEFAULT_TELEMETRY_CONSENT_ASKED, skipped_update_version: String = DEFAULT_SKIPPED_UPDATE_VERSION, timer_visible: bool = DEFAULT_TIMER_VISIBLE) -> void:
	var config := ConfigFile.new()
	...
	config.set_value(UI_SECTION, TIMER_VISIBLE_KEY, timer_visible)
