...
func _run() -> void:
	...
	_check(keeper.get_current_health() == 48 and world.damage_recorded == 100, "Keeper itself takes all 30 damage while its links are down")
	await _capture("keeper_vulnerable", "KEEPER / EXPOSED SUPPORT", "The Keeper takes full damage. Wards can return only after the delay and a fresh warmup.")
	_free_world()
