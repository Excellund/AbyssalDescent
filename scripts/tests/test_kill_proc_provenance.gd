...
func _test_kill_callback_contract() -> void:
	for mask in [0, DAMAGEABLE.KILL_PROC_SUPPRESS_FRACTURE, DAMAGEABLE.KILL_PROC_SUPPRESS_ECHO_PULSE]:
		...
		_check(player.dash_cooldown_left == 0.0 and target.velocity.is_zero_approx() and not target.hits.any(func(hit: Dictionary) -> bool: return hit.type == "edict_court"), "Mask %d retains Reaper while a notification alone cannot manufacture Edict's accepted-Kill Burst" % mask)
...
func _test_oath_accounting() -> void:
	...
	await _settle()
	var echo_action: Dictionary = player.new_combat_action("melee")
	DAMAGEABLE.apply_damage(victim, 20, player.INTERACTION_REGISTRY.damage_context(echo_action, "sovereigns_double", {"secondary": true, "raw_amount": 20.0, "damage_coefficient": 1.0}), 1)
	...
	_check(is_equal_approx(float(DAMAGEABLE.status_snapshot(neighbor, 1).mark_ratio), player.eclipse_mark_bonus_ratio) and neighbor.is_slowed() and neighbor.velocity.is_zero_approx() and neighbor.hits.any(func(hit: Dictionary) -> bool: return hit.type == "edict_court"), "Accepted secondary kills retain Eclipse and Edict's Burst/Slow without inventing an Attack or moving the neighbor")
