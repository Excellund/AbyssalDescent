...
func _run() -> void:
	if not OS.get_user_data_dir().begins_with(ProjectSettings.globalize_path("res://")) or not DirAccess.dir_exists_absolute("res://validation_fixtures"):
		push_error("Threadbinder integration requires the isolated gameplay regression helper")
		quit(1)
		return
	...
	print("[OK] Threadbinder integration: %d checks, %d failures" % [checks, failures.size()])
	quit(0 if failures.is_empty() else 1)

func _test_normal_unlocks() -> void:
	var roster := CHARACTER.get_launch_character_ids()
	check(roster.size() == 5 and roster.back() == NEW_CHARACTER, "Threadbinder is the fifth playable character")
	...
	check(not RunContext.set_selected_character_id(NEW_CHARACTER), "Threadbinder cannot be selected before the normal unlock")
	...
	check(RunContext.consume_just_unlocked_character_id().is_empty(), "Repeated Riftlancer clears do not reannounce Threadbinder")
	check(RunContext.set_selected_character_id(NEW_CHARACTER), "Unlocked Threadbinder is selectable through production RunContext")
	RunContext.meta_progress_profile = META.load_meta_progress()
	check(META.get_selected_character_id(RunContext.meta_progress_profile) == NEW_CHARACTER and META.get_unlocked_character_ids(RunContext.meta_progress_profile) == roster, "Threadbinder selection and access survive disk reload")
	check(META.get_character_highest_unlocked_tier(RunContext.meta_progress_profile, NEW_CHARACTER) == 0, "Threadbinder starts with independent Pilgrim progression")
	RunContext.award_run_clear_unlocks(NEW_CHARACTER, 0)
	check(META.get_character_highest_unlocked_tier(RunContext.meta_progress_profile, NEW_CHARACTER) == 1 and RunContext.consume_just_unlocked_character_id().is_empty(), "Threadbinder clears advance its own Bearing without a phantom next character")
	for bearing in ["pilgrim", "delver", "harbinger", "forsworn"]:
		check(OATHS.get_all_definitions().has("clear_threadbinder_" + bearing), "Threadbinder has the ordinary " + bearing + " clear Oath")
...
func _test_existing_profiles() -> void:
	...
	check(META.get_next_character_unlock_for_clear(loaded, "riftlancer") == NEW_CHARACTER, "Existing profiles receive Threadbinder from their next Riftlancer clear")
...
func _test_lobby_selection() -> void:
	...
	check(lobby.character_selector.get_tab_count() == 6 and lobby.character_selector.get_tab_title(4) == "Threadbinder", "Co-op lobby includes Threadbinder before Random Vessel")
	...
	check(lobby.local_character_id == NEW_CHARACTER, "Choosing the fifth lobby tab resolves Threadbinder")
	lobby.free()
	session.free()
...
func _test_character_checkpoint() -> void:
	...
	check(String(saved.get("current_character_id", "")) == NEW_CHARACTER, "Actual checkpoint records Threadbinder identity")
	RunContext.selected_character_id = "bastion"
	var resumed := _replace_player(saved)
	check(world.current_character_id == NEW_CHARACTER and resumed.active_character_id == NEW_CHARACTER and bool(resumed.get("passive_cross_stitch")), "Disk resume reapplies Threadbinder and Cross Stitch despite a changed menu selection")
	check(resumed.build_run_snapshot() == expected and resumed.get_current_health() == 37, "Resume preserves Threadbinder health, stats and learned shared powers")
	await _cleanup()
...
func _test_coop_packages(mode: String) -> void:
	...
	check(actors[0].active_character_id == NEW_CHARACTER and actors[1].active_character_id == NEW_CHARACTER, mode + ": both duplicate-character peers receive Threadbinder packages")
	check(bool(actors[0].get("passive_cross_stitch")) and bool(actors[1].get("passive_cross_stitch")), mode + ": both peers receive Cross Stitch")
	check(actors[0].player_body_color != actors[1].player_body_color and actors[0].damage == actors[1].damage, mode + ": duplicate Threadbinders retain distinct palettes and equal mechanics")
