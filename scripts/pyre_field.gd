...
func _draw() -> void:
	...
	draw_circle(Vector2.ZERO, radius, Color(1.0, 0.36, 0.1, (0.1 + spawn_boost * 0.08) * fade))
	draw_circle(Vector2.ZERO, radius * 0.72, Color(1.0, 0.62, 0.18, (0.08 + pulse * 0.04) * fade))
	draw_arc(Vector2.ZERO, radius - 4.0 + pulse * 3.0, 0.0, TAU, 40, Color(1.0, 0.78, 0.36, (0.38 + tick_boost * 0.32) * fade), 2.4)
	for ring_i in range(3):
		var inner_radius := radius * (0.34 + float(ring_i) * 0.18)
