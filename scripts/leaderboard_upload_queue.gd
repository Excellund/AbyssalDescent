...
const QUEUE_PATH := "user://leaderboard_upload_queue.save"
...
static func _new_entry_id() -> String:
	return "lb-%s-%s" % [str(_now_unix()), str(Time.get_ticks_usec())]
...
static func _pending_index_by_run_id(pending: Array, run_id: String) -> int:
	for i in range(pending.size()):
		var entry := pending[i] as Dictionary
		var payload := entry.get("payload", {}) as Dictionary
		var summary := payload.get("p_run_summary", {}) as Dictionary
		if String(summary.get("run_id", "")).strip_edges() == run_id:
			return i
	return -1
...
static func enqueue(payload: Dictionary) -> String:
	...
	if not run_id.is_empty():
		var existing_index := _pending_index_by_run_id(pending, run_id)
		if existing_index >= 0:
			var existing := pending[existing_index] as Dictionary
			existing["payload"] = payload.duplicate(true)
			existing["next_retry_unix"] = 0
			existing["last_error"] = ""
...
static func get_ready_entries(max_entries: int = 1) -> Array[Dictionary]:
	var queue_payload := load_queue()
	var pending := queue_payload.get("pending", []) as Array
