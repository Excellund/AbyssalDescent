...
func begin_for_new_room(objective_runtime: Node, profile: Dictionary) -> void:
	if is_instance_valid(objective_runtime):
		objective_runtime.begin_room_objective(profile)

func reset_and_begin_for_new_room(objective_manager: Node, objective_runtime: Node, profile: Dictionary) -> void:
	reset_for_new_room(objective_manager, objective_runtime)
	begin_for_new_room(objective_runtime, profile)
