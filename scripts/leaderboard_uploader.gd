...
func enqueue_run_summary(run_summary: Dictionary) -> void:
	var payload := LEADERBOARD_MODEL.build_submission_rpc_body(run_summary)
	if payload.is_empty():
		return
	LEADERBOARD_QUEUE.enqueue(payload)
	_on_tick()

func pending_count() -> int:
	return LEADERBOARD_QUEUE.pending_count()
...
func _on_tick() -> void:
	...
	if not _is_upload_enabled():
		return
	var endpoint := _submit_endpoint()
	if endpoint.is_empty():
		return
	var entries := LEADERBOARD_QUEUE.get_ready_entries(1)
	if entries.is_empty():
		return
	...
	var request_error := _request.request(endpoint, headers, HTTPClient.METHOD_POST, JSON.stringify(payload))
	if request_error != OK:
...
func _mark_retry(entry_id: String, attempt_count: int, error_message: String) -> void:
	...
	LEADERBOARD_QUEUE.mark_failure(entry_id, capped_attempt, next_retry_unix, error_message)
