	...
	for child in get_children():
		if child is CollisionShape2D:
			...
			if shape_node.shape is CircleShape2D:
				...
				(shape_node.shape as RectangleShape2D).size = Vector2(68.0, 68.0)
...
func _is_in_priority_attack_state() -> bool:
	return boss_state == ENEMY_STATE_ENUMS.BossState.TELEGRAPH or boss_state == ENEMY_STATE_ENUMS.BossState.ATTACK
...
func _apply_custom_network_runtime_state(custom_state: Dictionary) -> void:
	...
	if custom_state.has("charge_hit_applied"):
		charge_hit_applied = bool(custom_state.get("charge_hit_applied", charge_hit_applied))
