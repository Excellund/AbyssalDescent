...
$renderRoot = Join-Path $tempRoot ('abyssal-keeper-render-' + [guid]::NewGuid().ToString('N'))
...
function Invoke-HiddenGodot([string]$Label, [string[]]$Arguments) {
    ...
    $lines | Where-Object { $_ -match '^\[OK\]|^\[FRAME\]|^KEEPER_FRAMES=' } | Write-Output
}
...
try {
    ...
    Invoke-HiddenGodot 'render' @('--verbose', '--display-driver', 'windows', '--rendering-method', 'gl_compatibility', '--rendering-driver', 'opengl3', '--audio-driver', 'Dummy', '--windowed', '--resolution', '1280x720', '--position', '-20000,-20000', '--max-fps', '60', '--fixed-fps', '60', '--quit-after', '1000', '--script', 'res://validation_entry.gd', '--', 'res://scripts/tests/render_keeper.gd')
