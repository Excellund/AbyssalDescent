...
$renderRoot = Join-Path $tempRoot ('abyssal-attack-render-' + [guid]::NewGuid().ToString('N'))
...
function Invoke-HiddenGodot([string]$Label, [string[]]$Arguments) {
    ...
    $lines | Where-Object { $_ -match '^\[OK\]|^\[FRAME\]|^ATTACK_FRAMES=' } | Write-Output
}
...
try {
    ...
    Invoke-HiddenGodot 'render' @('--verbose', '--display-driver', 'windows', '--rendering-method', 'gl_compatibility', '--rendering-driver', 'opengl3', '--audio-driver', 'Dummy', '--windowed', '--resolution', '1280x720', '--position', '-20000,-20000', '--max-fps', '60', '--fixed-fps', '60', '--quit-after', '1000', '--script', 'res://validation_entry.gd', '--', 'res://scripts/tests/render_attack_feedback.gd')
    $manifestPath = Join-Path $renderRoot 'attack_frames/manifest.json'
    if (-not (Test-Path -LiteralPath $manifestPath)) { throw 'The GPU fixture did not complete its capture manifest.' }
    $manifest = Get-Content -LiteralPath $manifestPath -Raw | ConvertFrom-Json
    if ($manifest.frames.Count -ne 3 -or $manifest.failures.Count -ne 0) { throw 'The GPU fixture did not produce all three valid states.' }
    Write-Host "GPU: $($manifest.gpu)"
    Write-Host "Attack frames: $(Split-Path -Parent $manifestPath)"
} finally {
