...
- For per-actor buckets (for example stats/timelines keyed by peer ID), route writes through narrow setter helpers and reads through ensure/accessor helpers instead of ad hoc dictionary mutation at each call site.
...
- Build local playtests from a separate production copy with isolated editor data, never from validation fixtures. Verify the embedded package retains real autoloads and excludes tests; keep temporary version/update settings out of source configuration and inspect the package without launching the game.
