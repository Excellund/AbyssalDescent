...
- **Reward transition cleanup parity** — If room-clear progression is host-only but reward selection is opened via RPC on clients, clear enemy lingering effects (for example `enemy_lingering_effects` Pyre fields) inside the shared reward-open path so joiners do not keep stale hazards.
