...
- **Storm Crown:** "Dealing damage charges chain lightning." It currently accepts any damage type; its description must not suggest Electric-only input. Its inspection rules explain one count per foe per original action and one discharge per action.
...
- **Faultline Seal** (saved ID `pillar_convergence`): "Attack hits or Electric damage charge a fixed seal. It Bursts after 0.8s; your Field damage inside detonates it early at +50% damage." One charge per original action, one stationary seal, no charging while armed or during its 0.6-second rearm lock. Actions spent then cannot bank delayed charging; its Burst and descendants cannot plant another seal. A fuse marker is not an owned Field.
...
Character passives use `scripts/shared/character_passive_catalogue.gd` for short selection copy, one canonical concise paragraph shared by Build Details and the glossary, and compatibility metadata. The glossary passive text must exactly match the Build Details paragraph, including semantic spans; never maintain an expanded glossary version. The glossary has no Power Rules category or individual power-rule entries. Keep canonical keyword definitions there; power inspection remains in Build Details. Keep their damage basis separate from activation: Sigil Burst needs an accepted attack hit after Dash, while Veilstep Rhythm needs no Attack. Farline Focus accepts melee and charged Blast contacts, not every source with the attack-hit property. Passive Bursts deal damage without creating new attack hits, Electric damage, Fields or Pushes. Render explicit semantic spans through the same keyword catalogue as rewards.
...
Boons increase generic capability. Missions grant a permanent Boon and a temporary increase; Arcana establish the build, while boss rewards extend or convert it. The expanded roster has fifteen Boons, six Mission bonuses, twenty-three Arcana and ten boss rewards under the existing equal-random offer rules.
