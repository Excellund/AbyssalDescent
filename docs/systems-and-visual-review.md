...
- Tether tracks each player's own crossing, shows its full damage capsule, and cancels when an endpoint disappears or is launched. Ordered replica state expires safely. Seamlock bands stop ticking at expiry and count down on replicas; their target selection and following spiral retain existing behavior.
- Damage flashes retain their strength and timing at the screen edges while leaving central combat clear. Shared ownership checks also handle connecting and closed peers before querying network identity. Native transport teardown and actual rendered comparisons cover both changes.
- Archer arrows resolve the full simultaneous path of the arrow and its current target, stop at nearer cover or the live arena edge, and expire safely on replicas after lost updates. This preserves existing damage, speed, aim timing and target eligibility. Frozen arrows during Ruinous launches reseed their movement history before resuming, preventing delayed hits from an earlier crossing. Focused checks, real ENet and six inspected frames cover the correction.
- Private run helpers and menu controls now belong to the scene that creates them. Repeated death, retry, menu Continue and abandon no longer leave those Nodes behind. Borrowed World registries and the deliberately shared mapper remain usable. The actual-scene regression has 152 checks; 37 additional UI checks preserve Back-button behavior and error-message layout.
- Actual room entry accepts native, empty and serialized obstacle lists. This fixes the integration error that prevented Breakwater from spawning. The count-aware campaign check completes all 16 character/Bearing combinations, while actual room and co-op tests verify the enemy and cover are present.
- Pyre fire cannot apply a tick scheduled after its lifetime. Toll resolves simultaneous player/ring movement inside the actual active interval, preserving strict spoke gaps, normal dash bypass, damage and cadence. Ordered replica state expires, and aura/heavy slows use the existing owner RPC. Independent geometry, real Orbit paths, native co-op effects and six inspected rendered states cover the change.
- Entering a room clears pending dash movement and queued attacks before repositioning. The next arena survey cannot replay the previous gesture or create a shade from the reset. Real solo and co-op door, pause, focus and retry checks preserve ordinary dash timing, cooldowns and party collision behavior.
- Exhausted rewards expose Continue through existing progression and co-op readiness, without granting a pick. Actual acquisitions and resume retain their build/Oath evidence. Menu panels fit smaller viewports, Ascension toggles preserve owned focus, and long reward text plus Draft Compass's fourth choice fit with usable Skip/reroll controls. Real GUI input and rendered states cover the changes.
- Co-op movement resends stationary state after a dropped stop, rejects older or wrong-room/run samples and validates their actual owner. Retry ignores retired avatars before setting up collision relationships. Actual native transitions, repeated initial rewards and old-run replay cover both directions while preserving movement physics and interpolation values.
