...
Updated September 11, 2026. This is the current entry point for the development pipeline and recorded milestones. The local [ordered feedback board](../tools/feedback-board/README.md) determines the next development priority unless the user specifies otherwise. Feature documents retain their detailed rules and historical verification records; a file named `next-content-update.md` does not mean that feature is still unimplemented.

**Current release checkpoint:** the user requested committing and pushing the combined playtest, merging into `main`, and tagging `v0.7.0`. The [checkpoint record](release-v0.7.0.md) covers the September 11 workday, the approved Riot Depth integration, Static Wake's damage reduction and the matching normal desktop build. Earlier baseline and workday records below retain their original milestone context.
