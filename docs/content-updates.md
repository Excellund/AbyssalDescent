...
`.github/scripts/start_debug_playtest.ps1` exports the debug build and launches that same desktop executable. Debug delivery uses the packaged desktop game; temporary source sessions remain validation tools. Both export modes are verified. The desktop file currently contains normal build `dev-overnight-readability-20260909-012139`: regular menu and progression, with no debug powers granted. Its verified SHA256 is `7A1BD02404624DA437C88B3E3FC2E6DE110EFDD609085212154E66545D5C37BA`. The [overnight playtest notes](overnight-playtest-notes-20260909.md) cover the new content and useful feedback.
